...
- **Do not wrap the structured `claim_story_agent` as a tool of the
  orchestrator.** Tried and reverted: `AgentTool` runs its agent as a root
  agent (requires `mode='chat'`, ours is `single_turn`), and the
  `sub_agents` auto-wrapping route forces the inner LLM to re-emit the whole
  `ClaimStoryResult` JSON verbatim, which intermittently truncates or
  repetition-loops. `tool_context.actions.skip_summarization` is not a fix —
  it ends the entire invocation before the orchestrator can narrate and
  leaves the sub-agent as the session's current agent. Future specialists
  (Benefits Q&A, ROI Gatekeeper) should likewise expose their deterministic
  service layer as FunctionTools on the orchestrator; keep standalone
  structured agents for direct `/run` invocation only.
